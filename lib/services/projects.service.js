...
export async function deleteProjects(ids) {
  ...
  const result = await repository.deleteProjects(numericIds);
  if (!result.allFound) {
...
// Unfiltered snapshot used to hydrate the client dashboard once. All later
// filtering (status/category/priority/search) and count updates (create,
// update, delete) happen client-side against this snapshot instead of
// issuing a fresh query per click.
