...
The script installs PostgreSQL and the PostgreSQL client if they are missing, starts the PostgreSQL service, creates the local teaching database and account when needed, installs Node.js dependencies, prepares the schema and teaching seed, verifies the database, and starts Next.js.
...
## Ubuntu launch behavior

Run `./run_ubuntu.sh` from the repository root. The script checks Node.js 24, installs PostgreSQL if it is missing, starts PostgreSQL, prepares the `week6_portal` database and `week6` application user, installs npm dependencies, runs database setup and verification, and starts Next.js on port 3000.
